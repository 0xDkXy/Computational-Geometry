...
int main()
{
    ...
    int pos = 1;
    Comp2G::Point maxp(1e9+7,1e9+7);
    for (int i = 1; i < stk.size(); ++i) {
        if (maxp.y > v[stk[i]].y) {
            maxp = v[stk[i]];
            pos = i;
        } else if (maxp.y == v[stk[i]].y) {
            if (maxp.x > v[stk[i]].x) {
                maxp = v[stk[i]];
                pos = i;
            }
    ...
    std::cout << stk.size() - 1 << "\n";
    // std::cout << int(v[stk.back()].x) << " " << int(v[stk.back()].y) << "\n";
    int len = stk.size();
    for (int i = 0; i < len; ++i) {
        if ((i+pos) % len == 0) continue;
        std::cout << int(v[stk[(i+pos)%len]].x) << " " << int(v[stk[(i+pos)%len]].y) << "\n";
    }
    // std::cout << stk.size() << "\n";
    // if (stk.size() - 1 == n)
    //     std::cout << 1 << "\n";
