...
namespace Comp2G{
    ...
    std::vector<Point> getConvexPolygon(std::vector<Point> );
    double getLongest(std::vector<Point> );
};
double Comp2G::getLongest(std::vector<Point> v)
{
    int j = 2;
    double rlt = 0.0;
    int len = v.size();
    if (len < 3) {
        rlt = distance(v[0], v[1]);
        return rlt;
    }
    for (auto i = 0; i < len + 1; ++i) {
        while (std::abs(_cross(v[i]-v[(i+1)%len], v[i] - v[j])) <=
                std::abs(_cross(v[i]-v[(i+1)%len], v[i]-v[(j+1)%len]))) {
                    j = (j + 1) % len;
                }
        rlt = std::max(rlt, std::max(distance(v[i], v[j]), distance(v[(i+1)%len], v[j])));
    }
    return rlt;
}

std::vector<Comp2G::Point> Comp2G::getConvexPolygon(std::vector<Comp2G::Point> v)
{
    std::sort(v.begin(), v.end(), Comp2G::key2compare);
    std::vector<int> stk;
    stk.emplace_back(0);
    int n = v.size();
    std::vector<int> used(int(v.size()), 0);
    // std::cout << *(stk.end()) << "\n";
    for (int i = 1; i < n; ++i) {
        while (stk.size() > 1 &&
