...
namespace Comp2G{
    ...
    bool _isbetween(const Point &a, const Line &b)
    {
        if (a.x >= std::min(b.p[0].x, b.p[1].x) &&
            a.x <= std::max(b.p[0].x, b.p[1].x) &&
            a.y >= std::min(b.p[0].y, b.p[1].y) &&
            a.y <= std::max(b.p[0].y, b.p[1].y)) return true;
        return false;
    }

    bool _isOnSegment(Point &a, Line &b)
    {
        if (_isbetween(a, b))
            if (projection(b, a) == a)
                return true;
    ...
    bool isOnSegment(Point a, Line b) {return _isOnSegment(a, b);}
};
...
int main()
{
    ...
    while (q--) {
        ...
        int ans = 0;
        for (int i = 0; i < n; ++i) {
            Comp2G::vec2 a(qp, pv[i]), b(qp, pv[(i + 1) % n]);
            if (qp == pv[i] || qp == pv[(i+1)%n] ||
                Comp2G::isOnSegment(qp, Comp2G::Line(pv[i], pv[(i+1)%n]))) {
                ans = 1;
                break;
            }
            double tmp = Comp2G::cross(a, b);
            double val = Comp2G::dot(a, b) / (Comp2G::abs(a) * Comp2G::abs(b));
            if (Comp2G::doubleEqual(tmp, 0.0))
                continue;
            else if (tmp > 0)
                angle += (std::acos(val) );
            else 
        ...
        if (ans == 1)
            std::cout << ans << "\n";
        else if (Comp2G::doubleEqual(angle, 0.0))
            std::cout << 0 << "\n";
        else 
            std::cout << 2 << "\n";
        // std::cout << std::setprecision(10) << std::fixed << angle << "\n";
    }
    return 0;
}